...
class GraphConv(nn.Module):
    ...
    def __init__(self, n_hops, n_users, n_items, interact_mat, deg,
        ...
        # print("self.deg: ", deg)
        # print("tensor of deg : ", torch.from_numpy(deg))
        # print("tensor of deg1 : ", torch.from_numpy(deg) + 1e-7)
        # print("log of deg : ", torch.log(torch.from_numpy(deg) + 1e-7))
        # print("sigmoid of deg : ", torch.sigmoid(torch.log(torch.from_numpy(deg) + 1e-7)))
        ...
        # print(t_u.size(), type(t_u))
        # print(t_i.size(), type(t_i))
        self.user_t.data = t_u
        self.item_t.data = t_i

        # self.user_t.data.fill_(t_u)
        # self.item_t.data.fill_(t_i)
        # print("ini user_t: ", self.user_t.size(), self.user_t.data)
        # print("ini item_t: ", self.item_t.size(), self.item_t.data)
    ...
    def forward(self, user_embed, item_embed, mess_dropout=True, edge_dropout=True):
        ...
        # print("t_u: ", self.t_u)
        # print("t_i: ", self.t_i)
        # print("forward user_t: ", self.user_t, self.user_t.size())
        # print("forward item_t: ", self.item_t, self.item_t.size())
        ...
        # i_weight = (torch.exp(-self.t_i).to(self.dev) *
        #             torch.pow(self.t_i, torch.FloatTensor([0]).to(self.dev)).to(self.dev)
        #             / torch.FloatTensor([factorial(0)]).to(self.dev))
        ...
        for k in range(1, self.n_hops + 1):
            ...
            item_embedds = item_embedds * (torch.exp(-self.item_t).to(self.dev) *
                                           ...
                                           / torch.FloatTensor([factorial(k)]).to(self.dev))
            # user_embedds = user_embedds * (torch.exp(-self.t_u).to(self.dev) *
            #                                torch.pow(self.t_u, torch.FloatTensor([k]).to(self.dev)).to(self.dev)
            #                                / torch.FloatTensor([factorial(k)]).to(self.dev))
            # item_embedds = item_embedds * (torch.exp(-self.t_i).to(self.dev) *
            #                                torch.pow(self.t_i, torch.FloatTensor([k]).to(self.dev)).to(self.dev)
            #                                / torch.FloatTensor([factorial(k)]).to(self.dev))
            side_embeddings_cur = torch.cat([user_embedds, item_embedds], dim=0)
            # embs = embs.add(side_embeddings_cur)
...
class ApeGNN_Per_Deg(nn.Module):
    def __init__(self, data_config, args_config, adj_mat, deg):
        super(ApeGNN_Per_Deg, self).__init__()
